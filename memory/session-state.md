...
- Deploy: succeeded via `bash scripts/deploy.sh`; latest deploy commit `282c0b9` (`Update 2026-06-30T09:08:45Z — automated`) plus post-deploy public metadata fix commit `7c036ea`.
- Mover vs 06Z: **no numeric probability moves**. Pakistan-Afghanistan remains the main active rising lane at **40 critical**; global stayed rounded at **67**.
...
- Source caveat: `web_search`/Tavily failed HTTP 432 on 17/17 required searches; fallback used Google News RSS for all trackers + oil/energy + IAEA/UN + NATO/allied + emerging, direct UN/NATO/OCHA/EIA/IAEA/OPEC probes, terminal HTTP, Yahoo energy feed, and Polymarket cache. Official probe: UN News/Press OK, NATO public pages OK, OCHA public pages OK, EIA RSS OK, IAEA news/press 403, OPEC 403.
- Energy/markets: deploy refreshed Yahoo energy feed: Brent **$73.69**, WTI **$70.48**, gas **$3.210**, gold **$4029.20**. Hormuz/tanker-risk headlines are live, but shipping/traffic and oil pricing still reject full-waterway-stoppage pricing. Polymarket refreshed `2026-06-30T09:09:24Z`; mapped markets remain horizon-mismatched sanity flags, worst `russia_ukraine` about **97.5pp**.
...
- Tooling: added `scripts/cron_deep_scan_fallback.py` to collect Google News RSS + official probes when web_search is quota-blocked; committed in the 09Z deploy chain.
- Verification: JSON valid; canonical signal check clean; `index.html` contains `DoomsdayWatch // Command Deck`, `const state = {`, and `// ===== RENDER`; deploy/push succeeded. After mandatory memory logging, local `memory/*.md` plus Syncthing vault files are modified.
